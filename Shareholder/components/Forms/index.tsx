...
import { getShareholder, signshareholderDocuments } from "@/api/shareholder";
import { DocumentInterface, ShareholderDocuments } from "../Main";
...
const UploadFormTable = ({ shareholderDocuments, documents }: { shareholderDocuments: ShareholderDocuments, documents: DocumentInterface[] }) => {
  const [signPreview, setsignPreview] = useState("-");
  const [type, setType] = useState(shareholderDocuments.type);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    console.log(values);
    const resp = await signshareholderDocuments(shareholderDocuments.companyId, values.sign);
    if(resp.success) {
